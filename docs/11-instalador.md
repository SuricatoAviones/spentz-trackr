...
Luego abre la URL de la app; como todavía no hay `storage/installed`, Redirigirá automáticamente a `/install` con el wizard:

1. **Requisitos** — comprueba versión de PHP, extensiones y permisos de escritura.
2. **Base de datos** — elige SQLite, MySQL o PostgreSQL e ingresa host, puerto, nombre de BD, usuario y contraseña. SQLite solo pide el nombre del archivo.
3. **Aplicación** — nombre, URL pública, idioma y zona horaria; además los datos del **administrador** (nombre, correo, contraseña).
4. **Completado** — se escribe `.env`, se genera `APP_KEY`, se ejecutan migraciones y se crea el admin. Se crea el archivo `storage/installed` para marcar la instalación terminada. Al final muestra un botón para iniciar sesión.
...
```bash
cp .env.docker .env
# edita .env: DB_PASSWORD, ADMIN_EMAIL, ADMIN_PASSWORD, APP_URL, APP_KEY, etc.
...
- Con **`APP_INSTALL_MODE=headless`** (Docker/Dokploy) el instalador web `/install` responde **404** y el arranque del contenedor llama `app:install --no-interaction` automáticamente si falta `storage/installed` (script `docker/entrypoint.d/99-spentz-install.sh`). En cPanel no seteamos la variable y se usa el wizard.
- La lógica compartida del instalador vive en `App\Services\Installer`, usada tanto por el controlador web (`InstallController`) como por el comando CLI (`app:install`).
- El `.env` se escribe in-place (a partir de `.env.example` si no existe) y fuerzan `SESSION_DRIVER=database`, `QUEUE_CONNECTION=database` y `CACHE_STORE=database` para que no haga falta un servidor de caché/cola adicional.
